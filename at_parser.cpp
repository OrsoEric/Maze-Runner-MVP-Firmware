...
Parser *born_parser( U8 *cmd )
{
	...
	if (ret == NULL)
	{
		DPRINTF(0, "PARSER_ERROR_MALLOC_FAIL!\n");
		return NULL;
	}
		///Link and fill
	ret -> cmd 				= cmd;
	...
	DPRINTF(0, ">END born_parser. ret: %x\n", (U32)ret);
...
Parser_packet *born_packet( void )
{
	...
	*/
	...
	///	RETURN
	///--------------------------------------------------------------------------
	DPRINTF( 0, ">END born_packet. adr: %x\n",(U32)ret);
	return ret;
...
U8 packet_add_arg( Parser_packet *packet, U8 arg_descriptor )
{
	...
	DPRINTF( 0, ">packet_add_arg. adr: %x\n", (U32)packet);
	//If: bad parameters
...
inline U8 packet_calc_arg_index( Parser_packet *packet, U8 index )
{
	...
	///	CHECK AND INITIALIZATIONS
	///--------------------------------------------------------------------------
	DPRINTF( 0, ">packet_calc_arg_index. adr: %x, index: %d\n",(U32)packet, index);
	...
	while (t < index)
	{
		...
		u8t = packet -> arg[t];
		DPRINTF( 0, "\targ%d, arg_descriptor: %c: ", t, u8t);
	...
	DPRINTF( 0, ">END packet_calc_arg_index. ret: %d\n", arg_index);
	return arg_index;
...
U8 packet_get_u8( Parser_packet *packet, U8 index )
{
	...
	DPRINTF( 0, ">packet_get_u8. adr: %x, index: %d\n", (U32)packet, index );
	...
	DPRINTF( 0, ">END packet_get_u8. ret: %u\n", ret);
	return ret;
...
S8 packet_get_s8( Parser_packet *packet, U8 index )
{
	...
	DPRINTF( 0, ">packet_get_s8. adr: %x, index: %d\n", (U32)packet, index );
	...
	DPRINTF( 0, ">END packet_get_s8. ret: %u\n", ret);
	return ret;
...
U16 packet_get_u16( Parser_packet *packet, U8 index )
{
	...
	DPRINTF( 0, ">packet_get_u16. adr: %x, index: %d\n", (U32)packet, index );
	...
	DPRINTF( 0, ">END packet_get_u16. ret: %u\n", ret);
	return ret;
...
S16 packet_get_s16( Parser_packet *packet, U8 index )
{
	...
	DPRINTF( 0, ">packet_get_s16. adr: %x, index: %d\n", (U32)packet, index );
	...
	DPRINTF( 0, ">END packet_get_s16. ret: %u\n", ret);
	return ret;
...
Parser_error_codes parser_cmd_chk( U8 *cmd )
{
	...
	while ((index != 255) && (IS_BIT_ONE(flags, 0)))
	{
		...
		if (status.cnt == PARSER_IDLE)
		{
			...
			else if (data == (U8)0xff)
			{
				...
				//End While
				CLEAR_BIT( flags, 0 );
				DPRINTF( 0, "\tPARSER_ERROR_BAD_CMD_ID! in position: %d\n", index);
			}
		...
		else if (status.cnt == PARSER_ID)
		{
			...
			if ((index -cmd_index) == 1)
			{
				...
				else
				{
					...
					//End While
					CLEAR_BIT( flags, 0 );
					DPRINTF( 0, "\tPARSER_ERROR_SYNTAX_FIRST_BYTE! in position: %d\n", index);
				}
			}
			...
			else
			{
				...
				//End While
				CLEAR_BIT( flags, 0 );
				DPRINTF( 0, "\tPARSER_ERROR_INVALID_CHAR! in position: %d\n", index);
			}
		}	//End if: PARSER_ID
		...
		else if (status.cnt == PARSER_ARG)
		{
			...
			else
			{
				...
				//End While
				CLEAR_BIT( flags, 0 );
				DPRINTF( 0, "\tPARSER_ERROR_INVALID_ARG_DESCRIPTOR! in position: %d\n", index);
			}
		}	//End if: PARSER_ARG
		//Default case
		else
		{
			...
			CLEAR_BIT( flags, 0 );
			DPRINTF( 0, "\tPARSER_ERROR_UNDEFINED_STATE! in position: %d\n", index);
		}
		...
		if (IS_BIT_ONE(flags, 0) && (data == '\0') && (old == '\0'))
		{
			...
			CLEAR_BIT( flags, 0 );
			DPRINTF( 0, "\tPARSER_ERROR_MISPLACED_END! in position: %d\n", index);
	...
	if (index == 255)
	{
		...
		DPRINTF( 0, "\tPARSER_ERROR_MAX_LENGTH_EXCEEDED! in position: %d\n", index);
	}

	DPRINTF( 0, "\tscanned %d char in the dictionary\n", index);
	DPRINTF( 0, ">END parser_cmd_chk. ret: %x\n",ret);
...
U8 *parser_search_first( U8 *cmd, U8 data, U8 *num_hit )
{
	...
	DPRINTF(0,">parser_search_first cmd: %x, data: %x, num_hit: %x\n", (U32)cmd, data, (U32)num_hit);
	...
	{
		DPRINTF(0,"\tPARSER_ERROR_NULL_POINTER_PARAMETER!\n");
		return NULL;
	}
	...
	while (IS_BIT_ONE( flags, 0 ))
	{
		...
		{
			DPRINTF(0,"\tindex: %d, data: %x\n", index, cmd[index]);
			if ((PARSER_PEDANTIC_CHECK) && (index == 255))
			{
				//infinite cycle detected
				DPRINTF(0,"\tPARSER_ERROR_MAX_LENGTH_EXCEEDED!\n");
				//break mid cycle
				CLEAR_BIT( flags, 1 );
			}
			...
			DPRINTF(0,"\tskipped until terminator: index: %d, cmd: %x\n", index, cmd[index]);
			...
			if (cmd[index] == '\0')
			{
				//break mid cycle
				CLEAR_BIT( flags, 1 );
				DPRINTF(0,"\tdouble terminator found. Dictionary end\n");
			}
		...
		else
		{
			...
			DPRINTF(0,"\tPARSER_ERROR_UNDEFINED_STATE!\n");
			return NULL;
		}
	}	//End While: pass cycle
	DPRINTF(0,">END parser_search_first. num_hit: %d %d, ret: %x\n", hit_cnt, num_hit[0], (U32)ret);
	///--------------------------------------------------------------------------
	///	RETURN
	///--------------------------------------------------------------------------
...
Parser_error_codes parser_calc_arg( U8 *arg, U8 arg_descriptor, S8 num )
{
	...
	DPRINTF( 0, ">parser_calc_arg. arg adr: %x, arg_descriptor: %x, chiper: %d\n", (U32)arg, arg_descriptor, num );
...
Parser_packet *parser_exe( Parser *parser, U8 din )
{
	...
	{
		DPRINTF(0, ">parser_exe adr: %x, din: c**%c**\n", (U32)parser, (U8)din);
	}
	else
	{
		DPRINTF(0, ">parser_exe adr: %x, din: x**%x**\n", (U32)parser, (U8)din);
	}
	//if: null pointer parameter
	if (parser == NULL)
	{
	...
	parser_tmp = parser[0];
	DPRINTF(0, "\tpacket adr: %x\n",(U32)parser_tmp.packet);
	...
	if (din == '\0')
	{
		DPRINTF(0, "\tTerminator. Current id_index entries: %d\n",parser_tmp.num_id);
		//If: i was decoding an argument
		if (parser_tmp.status.cnt == PARSER_ARG)
		{
			//I'm done decoding
			parser_tmp.index++;
			DPRINTF(0, "\tTerminator after ARG\n");
		}
		...
		if (parser_tmp.num_id > 0)
		{
			...
			for (t = 0;t<parser_tmp.num_id;t++)
			{
				...
				if (cmd[parser_tmp.id_index[t] +parser_tmp.index] == '\0')
				{
					...
					{
						DPRINTF(0, "\tno packet exists. Create a new one\n");
						ret = born_packet();
					...
					{
						DPRINTF(0, "\tExisting packet. command had arguments\n");
					...
					DPRINTF(0, "\tValid Command decoded! ID: %d %d, adr: %x\n", cmd[parser_tmp.id_index[t]], ret -> id, (U32)ret);
					//break cycle
					t = 254;
				}
			}
	...
	{
		DPRINTF(0, "\tPARSER_IDLE. parser_tmp.index: %d\n",parser_tmp.index);
		...
		{
			DPRINTF(0, "\tAt least on valid entry was found! ");
	...
	{
		DPRINTF(0, "\tPARSER_ID. parser_tmp.index: %d\n",parser_tmp.index);
		...
		{
			DPRINTF(0, "\tdata is number or letter: %c\n", din);
				///Prune bad entries
			DPRINTF(0, "\tcurrent id_entries: %d, address: %x\n", parser_tmp.num_id,(U32)parser_tmp.id_index);
			...
			DPRINTF(0, "\tsurviving id_entries: %d, address: %x\n", parser_tmp.num_id,(U32)parser_tmp.id_index);
			//if: at least one parser_tmp.id_index entry survived the purge
			if (parser_tmp.num_id > 0)
			{
				DPRINTF(0,"\t at least one parser_tmp.id_index entry survived: ");
			...
			{
				DPRINTF(0,"\t no parser_tmp.id_index entries survived the purge\n");
				//Issue an FSM reset
				SET_BIT( flags, PARSER_FSM_RESET_BIT );
			}
	...
	{
		DPRINTF(0, "\tPARSER_ARG. parser_tmp.index: %d\n",parser_tmp.index);
		...
		if (IS_SIGN(din))
		{
			...
			{
				DPRINTF(0,"\tdetected sign during arg decoding\n");
				...
				if (din == '+')
				{
					//Positive number
					parser_tmp.status.arg_sign = 0;
					DPRINTF(0,"\tnew positive arg decoding\n");
				}
				else
				{
					//Negative number
					parser_tmp.status.arg_sign = 1;
					DPRINTF(0,"\tnew negative arg decoding\n");
				}
	...
	if (error_code != PARSER_OK)
	{
		switch (error_code)
		{
			case PARSER_ERROR_BAD_CMD_ID :
				DPRINTF(0, "\tPARSER_ERROR_BAD_CMD_ID\n");
				break;
			case PARSER_ERROR_UNDEFINED_STATE :
				DPRINTF(0, "\tPARSER_ERROR_UNDEFINED_STATE\n");
				break;
			case PARSER_ERROR_INVALID_CHAR :
				DPRINTF(0, "\tPARSER_ERROR_INVALID_CHAR\n");
				break;
			case PARSER_ERROR_INVALID_ARG_DESCRIPTOR :
				DPRINTF(0, "\tPARSER_ERROR_INVALID_ARG_DESCRIPTOR\n");
				break;
			case PARSER_ERROR_MISPLACED_END :
				DPRINTF(0, "\tPARSER_ERROR_MISPLACED_END\n");
				break;
			...
			case PARSER_ERROR_SYNTAX_FIRST_BYTE :
				DPRINTF(0, "\tPARSER_ERROR_SYNTAX_FIRST_BYTE\n");
				break;
			case PARSER_ERROR_NULL_POINTER_PARAMETER :
				DPRINTF(0, "\tPARSER_ERROR_NULL_POINTER_PARAMETER\n");
				break;
			case PARSER_ERROR_NULL_POINTER_IN_PARSER :
				DPRINTF(0, "\tPARSER_ERROR_NULL_POINTER_IN_PARSER\n");
				break;
			case PARSER_ERROR_NEG_INPUT_POS_ARG :
				DPRINTF(0, "\tPARSER_ERROR_NEG_INPUT_POS_ARG\n");
				break;
			case PARSER_ERROR_ARG_OVERFLOW :
				DPRINTF(0, "\tPARSER_ERROR_ARG_OVERFLOW\n");
				break;
			default:
				DPRINTF(0, "\tPARSER_ERROR_UNDEFINED_ERROR error_code: %d\n", error_code);
		}
		//Destroy arguments
		SET_BIT( flags, PARSER_FSM_BURY_ARG_BIT );
		//Reset the FSM
	...
	{
		DPRINTF(0, "\tDestruction of argument\n");
		//clear signal
	...
	{
		DPRINTF(0, "\tAdding a new argument to parser structure\n");
		//Clear flag
	...
	parser[0] = parser_tmp;
	DPRINTF(0, "packet adr: %x\n", (U32)parser -> packet);
	DPRINTF(0, ">END parser_exe. ret: %x\n", (U32)ret);
